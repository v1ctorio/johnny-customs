...
export default async function listSubmissions({ skip, take }: {
	...
	return database.select().from(submissions_table).offset(skip); // 	return database.select().from(submissions_table).where(eq(submissions_table.approved,status)).limit(take).offset(skip);
}